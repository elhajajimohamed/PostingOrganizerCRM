...
async function debugAuth() {
  console.log('🔍 Firebase Auth Debugging Tool');
  ...
  try {
    ...
    if (listUsersResult.users.length === 0) {
      ...
      console.log('📝 This means you need to create a user account first.');
    } else {
    ...
    if (error.code === 'PERMISSION_DENIED') {
      console.log('💡 This might be a permissions issue with your service account');
    } else if (error.code === 'INVALID_CREDENTIAL') {
      console.log('💡 This might be an issue with your Firebase credentials');
    }
  }
